...
class XarrayDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.variables.pop("constant_surface",None)
        ...
        self.xr_options = dict(engine=engine, cache=True, chunks='auto')
        ...
        # Remove duplicate timestamps that arise when monthly NetCDF chunks include
        # a lead-time buffer (the first N steps of the next month appended to each
        # chunk).  Keep the first occurrence; data values are identical at both.
        seen: set = set()
        deduped = []
        ...
        n_dropped = len(self.timestamps) - len(deduped)
        if n_dropped:
            print(f"XarrayDataset: dropped {n_dropped} duplicate timestamp(s) "
                  f"(monthly file-seam overlap).")
        self.timestamps = deduped
