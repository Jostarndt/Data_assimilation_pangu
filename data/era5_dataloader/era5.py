...
#from stats import geoarches_stats
...
class Era5Dataset(XarrayDataset):
    ...
    def __init__(
        self,
        path: str = "/srv/data/era_arches/era5_240/full",
        domain: str = "train",
...
class Era5Forecast(Era5Dataset):
    ...
    def __init__(
        ...
        geoarches_stats_path= os.path.dirname(__file__)
        if variables ==  None:
            ...
            pdb.set_trace()
            surface_mean,surface_std = calculate_variable_stats_direct(variables['surface'])
            level_mean, level_std = calculate_variable_stats_direct(variables['level'])
            #constant_surface_mean, constant_surface_std = calculate_variable_stats_direct(variables['constant_surface'], axis=(1,2))
            data_stats = {"surface_mean":torch.from_numpy(surface_mean),
                    "surface_std":torch.from_numpy(surface_std),
                    "level_mean":torch.from_numpy(level_mean),
                    "level_std":torch.from_numpy(level_std),
                    "const_surface_mean":torch.from_numpy(constant_surface_mean),
                    "const_surface_std":torch.from_numpy(constant_surface_std),
                    }
            torch.save(data_stats, norm_file_path)
            '''
            data_stats = torch.load(norm_file_path, weights_only=True)
            # normalization,
            if self.norm_scheme == "pangu":
                self.data_mean = TensorDict(
                    surface=data_stats["surface_mean"],
                    level=data_stats["level_mean"],
                )
                self.data_std = TensorDict(
                    surface=data_stats["surface_std"],
                    level=data_stats["level_std"],
                )

            # variable names
            # TODO: fix this below
            self.surface_variables = ["U10m", "V10m", "T2m", "SP", "TP24H"]
            self.level_variables = [
                a + str(p) for a in ["Z", "U", "V", "T", "Q"] for p in pressure_levels
            ]
